...
class Model(nn.Module):
	def __init__(self, lm_model, n_ent_tra,n_ent_ind, n_rel, model ,score_function,hr,tr,ht,hrt):
		super().__init__()

		self.lm_model = lm_model
		self.n_ent_tra = n_ent_tra
		self.n_ent_ind = n_ent_ind
		self.n_rel = n_rel
		self.model = model
	...
	def score_triples_mln(self, h_embs, r_embs, t_embs):
		cat = torch.cat([h_embs, r_embs,t_embs ], dim=-1)

...
class Trainer:
	...
	def train(self):
		...
		ent2id_tra = data_loader.ent2id_tra
		ent2id_ind = data_loader.ent2id_ind
		rel2id = data_loader.rel2id
		entity_list_tra = data_loader.entity_list_tra
		relation_list = data_loader.relation_list
		groundtruth_tra = data_loader.get_groundtruth_tra()
		...
		for epc in range(epoch):
			...
			for i_b, batch in tqdm(enumerate(data_sampler), total=n_batch,desc = 'train'):
				...
				real_inputs_h,real_inputs_t = data_loader.batch_tokenize(real_triples, "tra")
				...
				target_idxs = [ ent2id_tra[tar] for tar in targets]
				for i, triple in enumerate(real_triples):
					h, r, t = triple
					expects = set(groundtruth_tra['train']['tail'][(r, h)])
				...
				for i, triple in enumerate(real_triples):
					h, r, t = triple
					expects = set(groundtruth_tra['train']['head'][(r, t)])
					label_idx = [i_t for i_t, target in enumerate(targets) if target in expects]
					label_idx_list.append(label_idx)
			...
			logging.info('Train: Epoch: {} , Avg_Link_Prediction_Loss: {}, Time: {}'.format(epc, avg_loss_link_prediction, time_epoch))


			metrics_valid = self.link_prediction(epc, 'valid',"tra")
			metrics_test = self.link_prediction(epc, 'test',"ind")
	...
	def link_prediction(self, epc=-1,split='valid',modes = "tra"):
		model = self.model
		device = self.device
		hyperparams = self.hyperparams
		data_loader = self.data_loader
		if modes == "tra":
			n_ent = model.n_ent_tra
			ent2id = data_loader.ent2id_tra
			entity_list = data_loader.entity_list_tra
			dataset = data_loader.get_dataset_tra(split)
			groundtruth = data_loader.get_groundtruth_tra()
		else:
			n_ent = model.n_ent_ind
			ent2id = data_loader.ent2id_ind
			entity_list = data_loader.entity_list_ind
			dataset = data_loader.get_dataset_ind(split)
			groundtruth = data_loader.get_groundtruth_ind()
		n_rel = model.n_rel
		rel2id = data_loader.rel2id
		model.eval()
		sigmoid = torch.nn.Sigmoid()
		...
		with torch.no_grad():
			...
			for batch in batch_list:
				...
				target_inputs = data_loader.batch_tokenize_target(batch_targets,modes)
